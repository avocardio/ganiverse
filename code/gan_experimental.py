...
from gan_experiment import new_loss
...
def generator_loss(fake_output):
    return cross_entropy(tf.ones_like(fake_output), fake_output) + new_loss(5, 10)
...
def train(dataset, epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0 and epoch > 1:
            ...
            tf.saved_model.save(gen, SAVED_MODEL_PATH + 'generator_model_tf')
            print('Model saved.')
        ...
        if epoch == 250:
            ...
            tf.saved_model.save(gen, SAVED_MODEL_PATH_50 + 'generator_model_tf')
            print('Model saved.')
            send_text('Model saved.')
