...
def augment(input):
    ...
    for idx, input in tqdm.tqdm((enumerate(input))):
        ...
        # contour filter & invert to ensure black background
        #cont_img = img.filter(ImageFilter.CONTOUR)
        #cont_img = ImageOps.invert(cont_img)
        #cont_img = cont_img.save(f'{path}/{idx}_contour.jpg')
        #augmented_images.append(cont_img)
        ...
        #edge_img = edge_img.save(f'{path}/{idx}_edge.jpg')
